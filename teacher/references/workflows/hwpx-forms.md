...
HWPX는 ZIP 내부 XML이다. 이 문서는 **기본 경로가 아니라 예외 경로**다.

기본 경로는 `kordoc patch`다 — 원본 포맷 유지(`.hwp`→`.hwp`, `.hwpx`→`.hwpx`), 변환·COM 불필요
(`references/routing.md`, `references/kordoc.md` 참조).

이 문서는 다음 경우에만 읽고 적용한다.

- 원본이 이미 `.hwpx`이고 `fill`, `seal`, XML 직접 편집이 필요한 경우
- 사용자가 `.hwp`→`.hwpx` 변환 또는 구조 편집을 명시적으로 승인한 경우
- 단순 텍스트 패치로는 처리할 수 없는 이미지/서명 삽입, HWPX 전용 기능이 필요한 경우

일반 교사 행정문서 작업에서는 행 추가, 열 추가, 글자 크기 수정, 레이아웃 재설계를 기본으로 하지 않는다. 칸이 좁거나 내용이 길어도 임의 요약하지 말고 작성 의도를 우선해 넣는다.

## 기본 흐름

HWPX 구조 편집을 명시적으로 선택했을 때의 흐름:

원본 `.hwpx` 복사 또는 승인된 `.hwp` -> 임시 `.hwpx` 변환 -> HWPX ZIP/XML 직접 편집 -> `validate.py` 구조 검증 -> 주요 입력값 확인 -> 최종 `*_완성본.hwpx` 저장 -> 임시 파일 삭제

1. 원본이 이미 `.hwpx`이면 원본을 덮어쓰지 말고 복사본을 작업본으로 둔다.
2. 원본이 `.hwp`이면 기본으로 변환하지 않는다. 사용자가 변환을 승인한 경우에만 한글 COM으로 임시 `.hwpx` 작업본을 1회 만든다.
3. `scripts/clone_form.py --analyze 작업본.hwpx`로 문단, 표, 텍스트 조각을 확인한다.
4. 단순 기존 텍스트 치환이면 XML 직접 편집보다 `kordoc patch`로 되돌아간다.
5. 빈 표 셀, 이미지, 서명처럼 HWPX 내부 구조를 만져야 하면 `Contents/section0.xml`의 표/셀 구조를 분석하고 XML을 직접 수정한다.
6. 편집 결과는 최종 산출물로 `.hwpx`만 유지한다. 다시 `.hwp`로 저장하지 않는다.
7. `scripts/validate.py` 구조 검증과 `Contents/section0.xml` 직접 확인으로 주요 값이 유지되는지 확인한다.
8. 임시 변환본, 압축 해제 폴더, 임시 스크립트 산출물은 삭제한다.
...
- 검증용 HWPX 별도 생성
- PDF/이미지 렌더링 검증
- 행 추가, 열 추가, 글자 크기 수정, 레이아웃 재설계

## HWP를 임시 HWPX로 변환

이 절차는 기본 경로가 아니다. `.hwp`는 `kordoc patch`로 처리한다. 사용자가 `.hwpx` 변환을 명시적으로 승인한 경우에만, 원본 HWP를 편집 가능한 HWPX로 꺼내는 1회 변환을 시도한다.
...
HWPX 변환 실패 뒤 자동 우회를 하지 않는다. 실패 사실, 멈춘 단계, 원본 보존 위험을 사용자에게 짧게 알리고 다음 지시를 받는다.
...
- 가능한 선택지는 사용자가 변환된 HWPX를 제공하기, `kordoc patch`로 되돌아가기, 비기본 HWPML2X 복구 경로를 승인하기, 새 HWPX 양식 재작성을 승인하기 중 하나로 정리해 제안한다.

## 텍스트 추출

- 빠른 확인: `kordoc`로 md 추출(`npx -y kordoc@^3 문서.hwpx -o 문서.md`) 또는 `Preview/PrvText.txt`
...
- 새 줄 수가 원본 문단 수보다 많으면 임의 요약하지 말고 마지막 문단에 강제 줄바꿈으로 합친다. 검증 통과를 위해 임의 문단, 빈 run, XML 주석을 덧붙이지 않는다.
...
원본이 이미 `.hwpx`이면 `kordoc seal`(도장 배치)이 가장 단순한 경로다 (`references/kordoc.md`). 표·페이지가 밀리지 않게 앵커 문구 위에 부유로 얹는다. 아래 스크립트 경로는 `seal`로 처리하기 어려운 중첩표·복잡 구조나 세밀한 위치 조정이 필요할 때 쓴다.

개인정보 동의서, 확인서, 신청서처럼 하단에 `성명 : ... (인 또는 서명)` 문구가 있는 양식은 다음 순서가 안정적이다.

1. 원본이 `.hwpx`이면 복사본을 작업본으로 둔다. 원본이 `.hwp`이면 사용자가 HWPX 변환을 승인한 경우에만 한글 COM `Open` + `SaveAs(..., "HWPX")`로 임시 HWPX 작업본을 만든다.
...
- 내용을 임의로 축약하지 않는다.
- 한 문단을 긴 한 줄로 방치하지 말고 의미 단위마다 `<br>` 또는 문단을 사용해 읽기 좋게 넣는다.
- 행 높이, 페이지 흐름, 글자 겹침은 교사가 한글에서 최종 확인할 수 있게 둔다.
- 글자 크기 축소, 행 높이 조정, 열 너비 조정은 기본 작업 범위가 아니다.

## 검증

HWPX 구조 편집을 수행한 경우에만 구조와 주요 입력값을 확인한다. 검증용 HWPX를 별도로 만들거나, 완성본을 한컴 COM으로 재저장하거나, PDF/이미지 렌더링을 수행하지 않는다.
